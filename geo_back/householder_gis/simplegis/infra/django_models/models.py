...
def filter_points_in_distance(self, lon, lat, dist):
    point = Point(lon, lat)
    return self.filter(geometry__distance_lt=(point, Distance(km=dist)))


def calculate_distance(self, lon, lat):
    point = Point(lon, lat, srid=4326)
    return self.annotate(distance=Distance("geometry", point))


class HouseQuerySet(models.QuerySet):
    def get_quaters_in_R(self, lon, lat, dist):
        return filter_points_in_distance(self, lon, lat, dist).aggregate(
...
class MetroQuerySet(models.QuerySet):
    def get_stations_in_R(self, lon, lat, dist):
        return filter_points_in_distance(self, lon, lat, dist)


class BusStopQuerySet(models.QuerySet):
    def get_stops_in_R(self, lon, lat, dist):
        routes = set()
        filtered = filter_points_in_distance(self, lon, lat, dist)
...
class ShopQuerySet(models.QuerySet):
    def get_shops_in_R(self, lon, lat, dist):
        point = Point(lon, lat)
        return self.filter(geometry__distance_lt=(point, Distance(km=dist))).annotate(
            distance=Round(D("geometry", point))
        )


class House(models.Model):
    address = models.CharField(
        verbose_name="адрес", max_length=200, blank=False, null=False
    )
    quarters_count = models.PositiveIntegerField(
        verbose_name="количество_домохозяйств", blank=False, null=False
    )
...
class Metro(models.Model):
    ...
    line = models.CharField(
        verbose_name="линия_метрополитена", max_length=200, blank=False, null=False
    )
    incoming_passengers = models.PositiveIntegerField(
        verbose_name="трафик пассажиров", blank=False, null=False
    )
    geometry = models.GeometryField()
...
class BusStop(models.Model):
    name = models.CharField(
        verbose_name="название", max_length=100, blank=False, null=False
    )
    route_numbers = models.CharField(
        verbose_name="маршруты", max_length=600, blank=False, null=False
    )
    geometry = models.GeometryField()
...
class Shop(models.Model):
    ...
    square = models.FloatField(
        verbose_name="площадь, м2", default=0, blank=False, null=False
    )
    sales = models.FloatField(
        verbose_name="оборот, в руб./мес.", default=0, blank=False, null=False
    )
    geometry = models.GeometryField()
